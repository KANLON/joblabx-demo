...
/**
 * 文件上传的servlet(保存到临时表中，只分析当前上传的excel表)类
 *
 * @author zhangcanlong
 * @date 2018年11月12日
 */
public class FileUploadTempExcelServlet extends HttpServlet {

	private static final long serialVersionUID = 1L;

	// 上传文件存储目录
	private static final String UPLOAD_DIRECTORY = Constant.WEB_APP_ROOT + "WEB-INF/" + "tempfile/";
			...
			if (formItems != null && formItems.size() > 0) {
				...
				for (FileItem item : formItems) {
					...
					if (!item.isFormField()) {
						...
						// result));
						// out.flush();
						response.sendRedirect("year_and_school_temp.html");
						return;
					}
				}
			}
